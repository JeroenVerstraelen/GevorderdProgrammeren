...
Observer* ParticleObserver::notify(model::Entity* entity, Notification n){return nullptr;}

void ParticleObserver::notify(model::ParticleGenerator* particle_generator, Notification n) {
	switch(n){
		case MOVE: 
		{
			std::vector<model::Particle> particles = particle_generator->getParticles();
