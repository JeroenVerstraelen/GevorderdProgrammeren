...
Monster::Monster(std::vector<view::Observer*> obs, double x_location, double y_location, double collision_radius, double x_speed, double y_speed) 
...
std::vector<view::Observer*> Monster::notifyDestruct() {
	std::vector<view::Observer*> particle_observers;

	for(unsigned int i = 0; i < observers.size(); ++i) {
		view::Observer* particle_observer = observers[i]->notify(this, view::EXPLODE);	
