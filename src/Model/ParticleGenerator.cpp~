...
namespace si {
	...
	ParticleGenerator::ParticleGenerator(std::vector<Observer*> observers, unsigned int count, ParticleType type) : 
	...
	bool ParticleGenerator::update(double elapsed)
	{
		...
		std::vector<Observer*>::iterator observer_it;
		for(observer_it = _observers.begin(); observer_it != _observers.end(); ++observer_it) {
			ParticleObserver* particle_observer = static_cast<ParticleObserver*>(*observer_it);
			particle_observer->notify(this, MOVE);
		}
	...
	void ParticleGenerator::notifyDraw() {
		...
		for(observer_it = _observers.begin(); observer_it != _observers.end(); ++observer_it) {
			ParticleObserver* particle_observer = static_cast<ParticleObserver*>(*observer_it);
			particle_observer->notify(this, DRAW);
		}
	}
