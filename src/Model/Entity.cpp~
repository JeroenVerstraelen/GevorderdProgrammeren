...
Entity::Entity(std::vector<view::Observer*> obs, double x_location, double y_location, double collision_radius, double x_speed, double y_speed) : observers(obs), _x(x_location), _y(y_location),
...
void Entity::notifyDraw(){
	std::vector<view::Observer*>::iterator observer_it;	
	for(observer_it = observers.begin(); observer_it != observers.end(); ++observer_it)
		(*observer_it)->notify(this, view::DRAW);
}

std::vector<view::Observer*> Entity::notifyDestruct() {
	std::vector<view::Observer*> empty;	
	return empty;
}
...
std::vector<view::Observer*> Entity::getObservers() const {
	return observers;
}
