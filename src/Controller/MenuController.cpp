...
namespace si {
	...
	MenuController::MenuController(LevelWindow* lw)  : _window(lw) {
		level = new Level(std::make_pair(lw->getSize().x, lw->getSize().y));
	}
	...
	bool MenuController::tick(StopWatch* stop_watch) {
				...
				case sf::Event::KeyPressed:
					if (event.key.code == sf::Keyboard::Escape) {
						...
						if(stop_watch->getPlayerBulletTime() >= 250) {
							...
							// Shoot a bullet.
							Observer* bullet_observer = new EntityObserver(_window);
							try {
							...
							Bullet* bullet = new Bullet(bullet_observers, level->getPlayer()->getLocation().first, 
		...
		LevelStatus end_status = level->tick(stop_watch);
		if(end_status == FAILURE) {
			// Restart the simulation.
			delete level;
			level = new Level(std::make_pair(_window->getSize().x, _window->getSize().y));
			loadSim();
		}
		else if(end_status == SUCCES) {
			// Reward the player.
			Observer* logo_observer = new EntityObserver(_window);
			try {
				logo_observer->addTexture(_window->getTextures().at("logo"));
			}catch(std::out_of_range) {
				std::cerr << "Logo texture file not found." << std::endl;
			}
			std::vector<Observer*> logo_observers = {logo_observer};
			Button* logo = new Button(logo_observers, 580, 110, 0, 0);
	...
	bool MenuController::start() {
		...
		Observer* button_observer = new EntityObserver(_window);
		try {
		...
		std::vector<Observer*> button_observers = {button_observer};
		Button* b1 = new Button(button_observers, 600, 400, 475, 50);
